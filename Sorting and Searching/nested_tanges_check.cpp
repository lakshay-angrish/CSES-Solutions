...
int main() {
	...
	ll min_right = INT32_MAX;
	vll contains(n, 0);
	FORD (i, n - 1, -1) {
		if (sorted[i][1] >= min_right)	contains[sorted[i][2]] = 1;
		min_right = min(min_right, sorted[i][1]);
	}
	vout(contains);
 
	ll max_right = INT32_MIN;
	vll contained(n, 0);
	FOR (i, 0, n) {
		auto it = lower_bound(sorted.begin(), sorted.end(), sorted[i]);
		if (sorted[i][1] <= max_right)	contained[sorted[i][2]] = 1;
		max_right = max(max_right, sorted[i][1]);
	}
	vout(contained);
